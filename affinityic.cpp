...
// 	int *ir, *jc;
	...
	nr = mxGetM(in[0]);  /* all mxGet take the inputs */
	nc = mxGetN(in[0]);
    ...
    /* find my sigma (if no sigmaF is given, the program gives a sigmaF) 
     * [max(emag(:)) * dataW.edgeVariance] */
	if (nargin<5) {
	    sigma = 0;
    	for (k=0; k<np; k++) { 
    ...
    for (j=0; j<np; j++) {            
        ...
        for (k=pj[j]; k<pj[j+1]; k++) {
            /* pj is the number of neighbors */
            i = pi[k]; /* index of each pixel
            ...
            if (i==j) {
                ...
                if (abs(di) >= abs(dj)) {  
	                ...
	                for (ii=0;ii<abs(di);ii++){
	                    iip2 = iip1 + step;    /* increase step size */ 
	                    jjp2 = (int)(0.5 + slope*(iip2-jy) + jx); /* move along the line */
	  	  
	                    phase2 = ephase[iip2+jjp2*nr];
               
	                    if (phase1 != phase2) {
	                        z = (emag[iip1+jjp1*nr] + emag[iip2+jjp2*nr]); /* Compute total energy */
	                        if (z > maxori){
	                            maxori = z;
	                        }
	                    }
	            ...
	            /* sample in j direction */    
	                ...
	                for (jj=0;jj<abs(dj);jj++){
	                    ...
	                    if (phase1 != phase2){
	                        z = (emag[iip1+jjp1*nr] + emag[iip2+jjp2*nr]); /* Formula for |F(i)-F(j)|/sigmaF */
	                        if (z > maxori){ 
	                            maxori = z; 
	                        }
                ...
                maxori = 0.5 * maxori;
                maxori = exp(-maxori * maxori * a);  /* Exponential */
            }       
		    ir[total] = i;
